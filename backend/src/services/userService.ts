...
export class UserService {
	 ...
	 * @param {string} role - Rol del usuario ("admin", "gestor", "auditor")
	...
	static async createUser(email: string, password: string, role: string): Promise<User> {
		try {
			...
			//Guardamos el usuario y el rol en Firestore
			await setDoc(doc(db, 'users', user.uid), {
				email: email,
				role: role, // "admin", "gestor", "auditor"
				createdAt: new Date(),
			});
 ...
 // ------------------------------------------------------------- NO IMPLEMENTADO
	 ...
	 * @param userId - ID del usuario en Firebase Authentication
	 * @returns {Promise<string>} - El rol del usuario ("admin", "gestor", "auditor")
	 */
